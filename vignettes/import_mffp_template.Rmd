...
get <- function (endpoint,pars) {
  server <- "http://localhost:8080"

  url <- httr::modify_url(server, path = paste0("/api/v0",endpoint))
  response <- httr::GET(url, config = httr::add_headers("Content-type" = "application/json", "Authorization" = paste('Bearer',bearer)))

  return(response)
}

postCells <- function(lt) {
  endpoint <- "/cells"
  # Create resp
  responses <- list()
